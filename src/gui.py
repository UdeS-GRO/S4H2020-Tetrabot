...
class Gui:
    ...
    def __init__(self):
        ...
        Label(self.root, text='TETRABOT PROJECT',
              font='Helvetica 28').pack(padx=30, pady=10)

        canvas1 = Canvas(self.root, width=window_width,
                         height=window_height, bg='#C0C0C0')
        self.animation_frame = AnimationCanvas(self.root, canvas1)

        Button(self.root, text='Run', command=click_run).pack(
            side=LEFT, padx=10, pady=10)
        Button(self.root, text='Stop', command=click_stop).pack(
            side=LEFT, padx=10, pady=10)
        Button(self.root, text='Stand Up', command=click_stand).pack(
            side=LEFT, padx=10, pady=10)
        Button(self.root, text='Close', command=self.root.quit).pack(
            side=RIGHT, padx=10, pady=10)

        Checkbutton(self.root, text='Draw trajectory', variable=self.var_draw_trajectory).pack(
            side=RIGHT, padx=10, pady=10)
        Checkbutton(self.root, text='3D visualisation',
                    variable=self.var_3d).pack(side=RIGHT, padx=10, pady=10)
    ...
    def animate_step(self, step):
        self.animation_frame.display_step(
            step, self.var_draw_trajectory.get(), self.var_3d.get())
